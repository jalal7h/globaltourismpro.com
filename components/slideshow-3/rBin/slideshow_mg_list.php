...
function slideshow_mg_list(){
	...
	switch($_REQUEST['do']){
		case 'saveNew' : 
			slideshow_mg_saveNew();
			break;
		case 'saveEdit' : 
			slideshow_mg_saveEdit();
			break;
		case 'remove' : 
	...
	$list['target_url'] = '_URL."/?page=".$_REQUEST["page"]."&cp=".$_REQUEST["cp"]."&func=slideshow_mg_form&p=".$_REQUEST["p"]."&id=".$rw["id"]';
	...
	$list['remove_prompt'] = '__("آیا مایل به حذف اسلاید با عنوان ".$rw["name"]." هستید?")';
