...
function mg_dailytour_mg_list(){
	...
	switch ($_REQUEST['do']) {
		...
		case 'form':
			return mg_dailytour_mg_form();

		case 'saveNew':
			mg_dailytour_mg_saveNew();
			break;
		
		case 'saveEdit':
			mg_dailytour_mg_saveEdit();
			break;
	...
	$list['target_url'] = '_URL."/?page=admin&cp='.$_REQUEST['cp'].'&func='.$_REQUEST['func'].'&do=form&id=".$rw["id"]';
	...
	$list['list_array'][] = array( "head"=>lmtc("mg_dailytour_image:image") , "tag"=>"th", "picture" => 'mg_dailytour_image($rw["id"])[0]');
